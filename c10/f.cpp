...
int removeFrequnce(cv::Mat src);
int dftDemo(cv::Mat src);
int ifftDemo(cv::Mat srcMat);
int mouseROI(cv::Mat src);
...
int main()
{
    cv::Mat srcMat = imread("/Users/sum_young/private_code/CV/figure/2.jpg", 0);
    cv::imshow("src", srcMat);
    removeFrequnce(srcMat);
    return 0;
}